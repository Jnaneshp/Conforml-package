...
def plot_conformal_results(timestamps, observed, predicted, lower, upper, anomalies=None, title="Prediction Intervals and Anomalies", last_n=200):
    import matplotlib.pyplot as plt
    import numpy as np
    # Slice last_n points
    timestamps = np.array(timestamps)[-last_n:]
    observed = np.array(observed)[-last_n:]
    predicted = np.array(predicted)[-last_n:]
    lower = np.array(lower)[-last_n:]
    upper = np.array(upper)[-last_n:]
    plt.figure(figsize=(14, 6))
    plt.plot(timestamps, observed, label="Observed", color="black", linewidth=1)
    plt.plot(timestamps, predicted, label="Prediction", color="orange", linewidth=2)
    plt.fill_between(timestamps, lower, upper, color="red", alpha=0.4, label="Prediction Interval")
    if anomalies is not None:
        anomalies = np.array(anomalies)[-last_n:]
        plt.scatter(timestamps[anomalies], observed[anomalies], color="gold", s=40, label="Anomaly", zorder=5)
    plt.title(title)
    plt.xlabel("Time")
    plt.ylabel("Value")
    plt.legend()
